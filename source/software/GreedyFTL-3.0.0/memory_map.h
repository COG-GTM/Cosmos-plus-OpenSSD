...
#ifdef HOST_TEST
// Host unit tests: the FTL management region lives in one arena allocated by
// test/fakes/host_memory.c instead of at fixed Zynq DRAM addresses. Only the
// three absolute anchors change; every other address is derived from them
// exactly as in the firmware layout.
#include "host_memory.h"
#define FTL_MANAGEMENT_START_ADDR		HOST_TEST_DRAM_BASE_ADDR
#define DATA_BUFFER_BASE_ADDR 					HOST_TEST_DRAM_BASE_ADDR
#else
...
#ifdef HOST_TEST
#define COMPLETE_FLAG_TABLE_ADDR			(RESERVED_DATA_BUFFER_BASE_ADDR + HOST_TEST_RESERVED_DATA_BUFFER_SIZE)
#else
...
// for buffers
#ifdef HOST_TEST
#define DATA_BUFFER_MAP_ADDR		 		(TEMPORARY_PAY_LOAD_ADDR + HOST_TEST_TEMPORARY_PAY_LOAD_SIZE)
#else
