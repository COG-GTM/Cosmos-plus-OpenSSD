...
#ifdef HOST_TEST
// Host unit-test build: the FTL management region is an mmap'd host allocation
// (see test/stubs/host_memory.c). Fixed offsets are preserved relative to its base.
extern unsigned int hostDramBase;
#define FTL_MANAGEMENT_START_ADDR		(hostDramBase)
#define DATA_BUFFER_BASE_ADDR 					(hostDramBase)
#define COMPLETE_FLAG_TABLE_ADDR			(hostDramBase + 0x07000000)
#define DATA_BUFFER_MAP_ADDR		 		(hostDramBase + 0x08000000)
#else
