...
| `mocks/mock_nsc.*` | Replaces `nsc_driver.c`. Every `V2F*` NAND controller call is recorded; default behaviour is an ideal, always-ready NAND with clean ECC. Programmed pages are stored per (controller, way, row) and returned by later reads until the block is erased (`mock_nsc_page_data/spare`); never-programmed/erased pages read as `0xFF`, so a default boot has no bad-block table, scans the (clean) array and programs one (~1.5 s). Fault injection: controller busy, ready/busy masks, status-report values, transfer completion/ECC results, read-page source override. Call and IO write logs grow without bound. |
