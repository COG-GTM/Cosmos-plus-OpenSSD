...
| `mocks/mock_nsc.*` | Replaces `nsc_driver.c`. Every `V2F*` NAND controller call is recorded; default behaviour is an ideal, always-ready NAND with clean ECC. Fault injection: controller busy, ready/busy masks, status-report values, transfer completion/ECC results, read-page source data. |
...
* `InitBlockDieMap()` reads one console byte (`inbyte()`); the stub returns
  `'\n'` unless `ftl_test_queue_inbyte("X")` is used to force a bad-block scan.
